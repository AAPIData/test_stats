...
state_edu_table %>% filter()

 color_scheme <- c('#2C7BB6','#ABD9E9','#FFFFBF','#FDAE61','#D7191C')
# library(viridis)
...
highchart() %>%
  ...
  hc_colorAxis(min = 10, max=56,stops = color_stops(n=5, RColorBrewer::brewer.pal(5,"Spectral")),labels= list(format = "{value}%"),showInLegend=T) %>%
...
# hc_colorAxis(stops = color_stops(n=5, RColorBrewer::brewer.pal(5,"Spectral")),labels= list(format = "{value}%"),showInLegend=T) %>%
...
highchart() %>%
  ...
  hc_tooltip(pointFormat = tooltip_table(x,y,title), useHTML=T,  headerFormat= "")%>%
  hc_add_theme(hc_theme_538()) %>%
  hc_colorAxis(stops = color_stops(n=5, colors = colorRamps::blue2red(5)),labels= list(format = "{value}%"),showInLegend=T) %>%
   hc_legend(title = list(text= "Statewide Percentage", fontStyle ='italic'),align = "center",verticalAlign = "bottom",
            layout = "horizontal", padding = 5) 
